...
public:
    ...
    // @Return: success
    bool executePlacePose(const std::vector<double> &target_pose, int vel = 200, int acc = 1000, int opts = 2);

    // send pose target for HCR-12 to move to End Effector Target
    // @Arg: <place target pose>, <velocity, mm/s>, <acceleration, mm/s^2>, <mode>
    // @Return: success
    bool executePickPose(const std::vector<double> &target_pose, int vel = 200, int acc = 1000, int opts = 2);
